...
def get_intensity_transformer(size_in=64, n_down=2, n_filters=4, n_filters_max=16, batch_mean=True, name="intensity_transformer", dtype="float32"):
    input = Input(shape = (size_in, size_in, 1), name="{}_input".format(name), dtype=dtype)
    conv = input
    filters = n_filters
    for l in range(n_down):
        conv = Conv2D(filters, kernel_size = (3,3), strides=2, padding='same', activation='relu', name="{}_conv_{}".format(name, l))(conv)
        filters *=2
        if n_filters_max>0:
            filters = min(filters, n_filters_max)
    conv_flat = Flatten()(conv)
    offset = Dense(1, name = "{}_offset".format(name))(conv_flat)
    scale = Dense(1, name = "{}_scale".format(name))(conv_flat)
    if batch_mean:
        offset = K.mean(offset, axis=0)
        scale = K.mean(scale, axis=0)
    return Model(input, [offset, scale])

def plug_intensity_intensity_transformer(model, intensity_transformer_model, shared_input=True):
    if not shared_input:
