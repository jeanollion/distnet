...
class UnetEncoder():
    def __init__(self, n_down, n_filters, max_filters=0, image_shape=None, anisotropic_conv=False, use_self_attention=False, name="encoder_"):
...
def get_unet_model(image_shape, n_down, filters=64, max_filters=0, n_outputs=1, n_output_channels=1, out_activations=["linear"], anisotropic_conv=False, n_inputs=1, n_input_channels=1, use_self_attention=False, n_1x1_conv_after_decoder=0, use_1x1_conv_after_concat=True, n_stack=1, stacked_intermediate_outputs=True, stacked_skip_conection=True):
    ...
    encoders = [UnetEncoder(n_down, filters[0], max_filters[0], image_shape, anisotropic_conv, use_self_attention, name="encoder"+str(i)+"_") for i in range(n_stack)]
    decoders = [UnetDecoder(n_down, filters[1], max_filters[1], image_shape, anisotropic_conv, n_last_1x1_conv=n_1x1_conv_after_decoder, use_1x1_conv_after_concat=use_1x1_conv_after_concat, name="decoder"+str(i)+"_") for i in range(n_stack)]
